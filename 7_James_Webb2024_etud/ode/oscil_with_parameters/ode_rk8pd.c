/* Pour compiler le programme ode.c
	gcc -I /usr/extralib/include/ ode.c -o ode -lgsl -lgslcblas -lg2 -lm
...
{
  Phys_Param *phys = (Phys_Param*) params;
  double m   = phys->m;
  double k = phys->k;
  double x =yvec[0];
  double y =yvec[1];
...
/* controle de la precision de calcul */  
  ...
  gsl_odeiv_system sys = {fonction, NULL, DIM, &phys};
